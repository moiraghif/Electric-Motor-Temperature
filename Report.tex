...
In addition, to improve predictions of critical temperatures a custom weighted version of MSE is used as described in the previous section.
...
However, comparing results using only the loss value is not satisfying for the following task: the model have to be light enough to be easily used by a car. So both number of parameters and performance are considered and plotted in the Figure \ref{fig:banana}. %fede dovrà scegliere nome decente
